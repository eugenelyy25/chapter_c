...
int main(void)
{
    ...
    //Leibniz series loop
    //Continue until pi approximation, close enough to target_pi
    while (round(pi_approx*pow(10, precision))/pow(10, precision) != target_pi) {
        term = pow(-1, n) / (2 * n + 1);
        pi += term;
...
//  while (round(pi_approx * pow(10, precision)) / pow(10, precision) != target_pi)
    ...
    != target_pi: Compares rounded value with target value π (3.1415927).
